...
A private stream is a multi-party message stream on Nostr, built by sharing one private key between all participants. A stream can also split read and write capability: readers hold one key that opens events, while writers hold a separate key that signs and publishes them (§2).

In the ordinary shared-key form, participants sign NIP-59 giftwrap events with the shared key and publish them to relays. Anyone holding the key can query the stream by its public key:
...
## 2. Split Read/Write Streams

Some planes need everyone to read but only a smaller set to publish. A split stream keeps the same wrap-and-seal event shape, but uses two keypairs:

```
read_sk,  read_pk    // held by every reader; opens the stream
write_sk, write_pk   // held only by writers; addresses and signs the stream
```

The relay coordinate is the **write** pubkey:

```json
{ "kinds": [1059], "authors": ["<write_pk>"] }
```

The wrap is encrypted with the NIP-44 conversation key between the writer and the read key:

```
conv_key = nip44_conversation_key(write_sk, read_pk)
         = nip44_conversation_key(read_sk, write_pk)
```

A writer encrypts with `write_sk -> read_pk` and signs the outer wrap as `write_pk`. A reader decrypts with `read_sk -> write_pk`. Holding only `read_sk` is enough to discover and read the stream, but not enough to produce an event whose outer `pubkey` is `write_pk`.

```jsonc
{
  "kind": 1059,
  "pubkey": "<write_pk>",
  "content": nip44_encrypt(write_sk<->read_pk, {
    "kind": 20013,
    "pubkey": "<real author's pubkey>",
    "content": "<seal content, as defined by the layered protocol>",
    "tags": [],
    "created_at": 1686840217,
    "sig": "<real author's signature>"
  }),
  "tags": [ ["p", "<ephemeral pubkey>"] ],
  "created_at": 1686840217,
  "sig": "<write key signature>"
}
```

A receiver of a split stream MUST:

- Query the expected `write_pk`, never the `read_pk`.
- Verify the outer wrap signature under `write_pk`.
- Decrypt the wrap with `nip44_conversation_key(read_sk, write_pk)`.
- Apply the same seal-kind, binding, and layered-protocol checks as a shared-key stream.

The write key is only a spam gate and stream-address capability. It does not make the inner actor authoritative. Protocols layered over split streams MUST still verify the real signer inside the seal before honoring any action.
