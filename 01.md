...
> **Seal, or a plaintext inner.** Encrypting the rumor inside the seal (as the example does) makes it a non-standalone artifact, so no relay, honest or malicious, can retain and display it as a public event. But a signature over that encrypted seal is bound to the key that encrypted it, and breaks if the content is re-encrypted under a different key. So a stream that must carry a *signed* event across a key rotation drops the seal and places its signed rumor directly inside the wrap as plaintext, where the signature rides the rumor itself and survives re-wrapping; one that wants the leak-resistance keeps the encrypted seal. Which form applies is a fixed property of the protocol layered over the stream, per context, never a per-message choice.
