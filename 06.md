...
Non-ratcheted, asynchronous key rotation for post-removal secrecy and post-demotion write revocation.

**Rekeys** rotate a Channel's encryption key to cryptographically sever a removed user (or many), or rotate a split stream's write key to stop a demoted writer from publishing. A single-channel read rekey typically follows removing someone's access to a specific channel — e.g. removing a role (`Tester`) with access to a private channel (`#testers`). A write-only rekey follows demoting someone who may keep reading but must lose publish capability.
...
A rekey has a scope: a **Channel read key**, a **Channel write key**, the **Control write key**, or the **`community_root`** (a base rotation):
```
Scope       = ChannelRead(channel_id) | ChannelWrite(channel_id) | ControlWrite | community_root
id32(Scope) = channel_id[32]          | sha256("concord/scope/channel-write" || channel_id)
            | sha256("concord/scope/control-write" || community_id)
            | 0x00…00 (all-zeroes, never collides with a Channel ID)
```

Private Channels are independently keyed (cryptographically unrelated to the `community_root`, CORD-03), so a compromised Private Channel exposes only itself and can be rekeyed alone. A Public Channel derives its read key from the `community_root`, so it has no independent read rekey — it rotates only when the base does (a Refounding). Restricted-write Channels and the Control Plane have independent write keys, so demoting a writer can rotate only the write key while leaving read access intact.
  ...
  "content": "nip44_encrypt(conv_key, {   // seal
     ...
     "content": "nip44_encrypt(conv_key, { // rumor
        ...
        "tags": [
          ...
          ["prevepoch",  "<decimal>"],
          ["prevcommit", "<hex>"],
          ["writepk",    "<new write pubkey>"],  // write-key scopes only: lets readers follow the new address
...
```
The wrapped plaintext is fixed-width, 72 bytes: `scope_id[32] ‖ epoch_be[8] ‖ new_key[32]`. The scope and epoch live *inside* the ciphertext, and a recipient verifies both against the event's tags before accepting the key, making a blob unspliceable. For split streams, `new_key` is the new write secret; the encrypted rumor also carries `writepk`, the resulting public write coordinate, so readers excluded from the write rotation can follow the new address without receiving the secret. The wrap key is the NIP-44 conversation key between the Rotator and the recipient — one ECDH either side can compute, so a NIP-46 bunker account opens its blob with a single nip44_decrypt, no raw-key access needed.

## 2. Receiving & Processing Rekeys

A receiver detects whether they've been removed from the channel or community, or merely excluded from a write-key rotation. The client then either visibly removes the lost read scope, keeps reading with a new public write coordinate, or switches to the new secret key when it received one.
...
```jsonc
{
  "kinds": [1059],
  "authors": [
    // per PRIVATE Channel read key you hold; the NEXT channel-epoch's rekey address:
    group_key("concord/rekey-pseudonym", community_root, channel_id, channel_epoch + 1).pk,
    // per restricted-write Channel write key you hold; the NEXT write epoch's rekey address:
    group_key("concord/rekey-pseudonym", community_root, channel_write_scope_id, write_epoch + 1).pk,
    // if you hold Control write capability; the NEXT Control write epoch's rekey address:
    group_key("concord/rekey-pseudonym", community_root, control_write_scope_id, control_write_epoch + 1).pk,
...
- If any chunk contains your locator, decrypt the new key and shift to the new epoch.
- Only once you hold **all `n` chunks** and none contains your locator have you been removed from that scope. For a read scope, the client loses access. For a write scope, the client keeps reading but drops its stale write secret and follows the `writepk` carried by the rotation. A missing chunk is never a removal — the client refetches until the set is complete before concluding anything.
...
- The Control write key is rolled with the base, so the Control Plane address rotates along with the read epoch.
- The compacted Control Plane is republished under the new `community_root` — only after confirmed publication of the root roll.
- All Private Channel read keys and restricted-write Channel write keys relevant to the removed user(s) are rekeyed, each sealed and addressed under the **prior** `community_root`, never the freshly minted one (Public Channel read keys rotate with the base for free, CORD-03). This matters under a race: if two Refoundings collide, the base converges on one winner and the losers drop their new roots, so a channel rekey sealed under a *new* root would be unreadable to every base-fork loser; sealing under the shared prior root keeps it openable on either branch.
...
**Authority.** A Channel read or write Rekey requires `MANAGE_CHANNELS`; a Control write Rekey requires the authority for the change that caused it (typically `MANAGE_ROLES` for a demotion); a Refounding requires `BAN`. In all cases the Rotator must strictly outrank every removed target (CORD-04). A receiver verifies the seal's real npub against its folded Roster before honoring anything, and a rotation cites the Grant it acts under like any authority action (CORD-04's `vac`), so a just-demoted admin's rotation is never honored by a lagging client. Holding a key is never authority: a removed member still holding the prior root can *construct* a perfectly shaped rotation, and every honest member opens the seal, folds the Roster, and drops it.
