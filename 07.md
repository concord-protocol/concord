...
where `(channel_secret, epoch)` is the Channel's read secret: `(community_root, root_epoch)` for a Public Channel and `(channel_key, channel_epoch)` for a Private one, exactly as in CORD-03. Restricted-write Channels use the same read secret for calls; their write key never gates media access.
