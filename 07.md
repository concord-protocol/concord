...
A voice Channel is any Channel whose ChannelMetadata (CORD-03 §2) carries `"voice": true`, edited like any other channel property (`MANAGE_CHANNELS`, CORD-04). Two sub-keys derive from the Channel's secret through the frozen shapes of CORD-02 Appendix A, fed by the same secret and epoch that address its Chat Plane (CORD-03 §1):
