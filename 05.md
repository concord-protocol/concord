...
CommunityInvite {
  ...
  control_write_pk, control_write_epoch,  // where readable Control Plane events are authored
  control_write_key?,                     // optional: present only for invitees who may publish Control editions
  channels: [{ id, key, epoch, name,
               write_pk?, write_epoch?, write_key? }],   // granted Channels; write_key only for writers
...
```
The inviter's own identity is irrelevant to trust: the `community_id` self-certifies the owner (CORD-02), so a bundle can't smuggle a false owner or a fake key for a real Community. A bundle whose `owner` and `owner_salt` fail to reproduce the `community_id` is refused. A bundle may give the recipient read access without write capability: `control_write_pk` and any channel `write_pk` identify where to read split streams, while `control_write_key` and `write_key` are included only when the recipient should publish to those planes.
