...
```
The inviter's own identity is irrelevant to trust: the `community_id` self-certifies the owner (CORD-02), so a bundle can't smuggle a false owner or a fake key for a real Community. A bundle whose `owner` and `owner_salt` fail to reproduce the `community_id` is refused.
